...
- Optional: provide `format=rgb565` that returns a raw RGB565 frame (little‑endian) with `Content-Type: application/octet-stream` for zero‑decode on the device. Include `X-Width`, `X-Height` headers.
