...
interchangeable. This guide covers the **M5Stack Tough K034**. Other M5
targets have their own profiles; see the [form-native beta targets](hw-reference/m5-form-native-betas.md)
and [AtomS3 + Joystick](hw-reference/board-atom-s3-joystick.md).
Do not inherit Tough hardware assumptions for another board.
...
- USB-UART bridge commonly appears on macOS as
  `/dev/cu.wchusbserial*`.
- The firmware uses `m5stack/M5Unified` and `m5stack/M5GFX` through the
  shared M5 platform component. The Waveshare Dial, Frame, and RLCD builds
  exclude that component; M5Stack Dial is a separate M5 target.
