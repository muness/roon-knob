...
static lv_obj_t *
    s_volume_label_large; // Volume display (large, prominent) - primary display
static lv_timer_t
    *s_volume_emphasis_timer; // Timer to reset volume emphasis after adjustment
...
static char s_zone_picker_ids[MAX_ZONE_PICKER_ZONES]
                             [MAX_ZONE_ID_LEN]; // Store zone IDs
static int s_zone_picker_count = 0;
static int s_zone_picker_selected = 0; // Currently highlighted item
static int s_zone_picker_current =
    -1; // Currently active zone (no-op if selected)
...
static int s_update_progress =
    -1; // Update download progress (-1 = not updating)
...
static char s_network_status[128] =
    ""; // Persistent network status (doesn't auto-clear)
...
static float s_last_predicted_volume =
    -9999.0f; // Track user's predicted volume for emphasis suppression
#ifdef ESP_PLATFORM
static ui_jpeg_image_t
    s_artwork_img; // Decoded RGB565 image for artwork (ESP32)
#else
...
static inline const lv_font_t *font_icon_small(void) {
  return font_manager_get_icon_small();
}
...
static inline void format_volume_text(char *buf, size_t len, float volume,
                                      float volume_min, float volume_step) {
...
static inline int calculate_volume_percentage(float volume, float volume_min,
                                              float volume_max) {
...
void ui_init(void) {
  ...
  // Periodic battery check for percentage drift (charging state changes trigger
  // immediate updates)
...
static void create_styles(void) {
  ...
  lv_style_set_bg_color(&style_button_primary,
                        lv_color_hex(0x2c2c2c)); // Dark grey
  lv_style_set_bg_opa(&style_button_primary, LV_OPA_COVER);
  lv_style_set_border_width(&style_button_primary, 3);
  lv_style_set_border_color(&style_button_primary,
                            lv_color_hex(0x5a9fd4)); // Light blue
  ...
  lv_style_set_bg_color(&style_button_secondary,
                        lv_color_hex(0x1a1a1a)); // Darker grey
  ...
  lv_style_set_text_color(&style_button_label,
                          lv_color_hex(0xfafafa)); // Off-white
}
...
static void build_layout(void) {
  ...
  lv_obj_add_flag(s_artwork_image,
                  LV_OBJ_FLAG_HIDDEN); // Hidden until artwork loads
  // Dim the artwork for better text contrast (avoid overlay layer)
  lv_obj_set_style_img_opa(s_artwork_image, LV_OPA_40,
                           0); // 40% opacity = 60% dimming
  ...
  lv_arc_set_bg_angles(s_volume_arc, 0,
                       359); // Nearly full circle (360 causes rendering issues)
  ...
  lv_obj_set_style_arc_color(s_volume_arc, lv_color_hex(0x3a3a3a),
                             LV_PART_MAIN); // Lighter grey for visibility
  lv_obj_set_style_arc_color(s_volume_arc, lv_color_hex(0x5a9fd4),
                             LV_PART_INDICATOR);
  ...
  lv_obj_set_style_arc_color(s_progress_arc, lv_color_hex(0x2a2a2a),
                             LV_PART_MAIN); // Slightly lighter
  lv_obj_set_style_arc_color(s_progress_arc, lv_color_hex(0x7bb9e8),
                             LV_PART_INDICATOR);
  ...
  lv_obj_set_size(header, SCREEN_SIZE - 60,
                  95); // Tall tap zone extending toward volume
  ...
  lv_obj_set_flex_align(
      ...
      LV_FLEX_ALIGN_CENTER); // Content at top, tap zone extends down
  lv_obj_set_style_pad_row(header, 0,
                           0); // Tight - battery and zone close together
  ...
  lv_obj_set_style_text_font(s_battery_icon, font_manager_get_lucide_battery(),
                             0);
  ...
  lv_obj_set_flex_align(now_playing, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER,
                        LV_FLEX_ALIGN_CENTER);
  lv_obj_set_style_pad_row(now_playing, 6, 0);
  lv_obj_align(now_playing, LV_ALIGN_CENTER, 0,
               20); // Centered with slight down offset
  ...
  lv_obj_set_style_margin_bottom(s_volume_label_large, 4,
                                 0); // Extra gap below volume
  ...
  lv_obj_set_style_bg_color(s_btn_prev, lv_color_hex(0x1a1a1a),
                            LV_STATE_DEFAULT);
  lv_obj_set_style_bg_color(s_btn_prev, lv_color_hex(0x3c3c3c),
                            LV_STATE_PRESSED);
  lv_obj_set_style_border_color(s_btn_prev, COLOR_GREY, LV_STATE_DEFAULT);
  lv_obj_set_style_border_color(s_btn_prev, lv_color_hex(0x5a9fd4),
                                LV_STATE_PRESSED);
  ...
  lv_obj_set_style_bg_color(s_btn_play, lv_color_hex(0x2c2c2c),
                            LV_STATE_DEFAULT);
  lv_obj_set_style_bg_color(s_btn_play, lv_color_hex(0x3c3c3c),
                            LV_STATE_PRESSED);
  lv_obj_set_style_border_color(s_btn_play, lv_color_hex(0x5a9fd4),
                                LV_STATE_DEFAULT);
  lv_obj_set_style_border_color(s_btn_play, lv_color_hex(0x7bb9e8),
                                LV_STATE_PRESSED);
  ...
  lv_obj_set_style_bg_color(s_btn_next, lv_color_hex(0x1a1a1a),
                            LV_STATE_DEFAULT);
  lv_obj_set_style_bg_color(s_btn_next, lv_color_hex(0x3c3c3c),
                            LV_STATE_PRESSED);
  lv_obj_set_style_border_color(s_btn_next, COLOR_GREY, LV_STATE_DEFAULT);
  lv_obj_set_style_border_color(s_btn_next, lv_color_hex(0x5a9fd4),
                                LV_STATE_PRESSED);
  ...
  lv_obj_set_style_text_color(s_status_bar, lv_color_hex(0x000000),
                              0); // Black text
  lv_label_set_long_mode(s_status_bar, LV_LABEL_LONG_DOT);
  // Background styling (hidden by default, shown when message appears)
  lv_obj_set_style_bg_color(s_status_bar, lv_color_hex(0xfafafa),
                            0);                            // Off-white
  ...
  lv_obj_align(s_status_bar, LV_ALIGN_BOTTOM_MID, 0,
               -25); // Higher up from edge
}
...
static void btn_prev_event_cb(lv_event_t *e) {
  (void)e;
  ESP_LOGI(UI_TAG, "btn_prev_event_cb triggered, s_input_cb=%p",
           (void *)s_input_cb);
  if (s_input_cb) {
...
static void btn_play_event_cb(lv_event_t *e) {
  (void)e;
  ESP_LOGI(UI_TAG, "btn_play_event_cb triggered, s_input_cb=%p",
           (void *)s_input_cb);
  if (s_input_cb) {
...
static void btn_next_event_cb(lv_event_t *e) {
  (void)e;
  ESP_LOGI(UI_TAG, "btn_next_event_cb triggered, s_input_cb=%p",
           (void *)s_input_cb);
  if (s_input_cb) {
...
static void apply_state(const struct ui_state *state) {
  ...
  if (s_track_label && s_artist_label) {
    ...
    ESP_LOGE(UI_TAG, "Label pointers are NULL! track=%p artist=%p",
             s_track_label, s_artist_label);
  }
  ...
  float vol_diff = state->volume < last_volume ? last_volume - state->volume
                                               : state->volume - last_volume;
  if (volume_initialized && vol_diff > 0.01f) {
    ...
    float pred_diff = state->volume < s_last_predicted_volume
                          ? s_last_predicted_volume - state->volume
                          : state->volume - s_last_predicted_volume;
    if (pred_diff > 0.01f) {
  ...
  int vol_pct = calculate_volume_percentage(state->volume, state->volume_min,
                                            state->volume_max);
  lv_arc_set_value(s_volume_arc, vol_pct);

  // Display volume (format matches zone's step precision)
  char vol_text[16];
  // Note: volume_min is atomic float read; no lock needed (self-corrects on
  // next poll if stale)
  format_volume_text(vol_text, sizeof(vol_text), state->volume,
                     state->volume_min, state->volume_step);
  ...
  if (s_play_icon) {
    ...
    lv_label_set_text(s_play_icon,
                      state->playing ? LV_SYMBOL_PAUSE : LV_SYMBOL_PLAY);
#endif
  }
...
static void poll_pending(lv_timer_t *timer) {
  ...
  if (network_status_changed && s_status_bar) {
    ...
    lv_obj_set_style_bg_opa(s_status_bar,
                            net_status[0] ? LV_OPA_COVER : LV_OPA_TRANSP, 0);
...
static void update_battery_display(void) {
  ...
  if (charging) {
    ...
    switch (level) {
    case 0:
      lv_label_set_text(s_battery_icon, ICON_BATTERY_WARNING);
      break; // Critical
    case 1:
      lv_label_set_text(s_battery_icon, ICON_BATTERY_LOW);
      break; // Low
    case 2:
      lv_label_set_text(s_battery_icon, ICON_BATTERY_MEDIUM);
      break; // Medium
    default:
      lv_label_set_text(s_battery_icon, ICON_BATTERY_FULL);
      break; // High
...
void ui_update_battery(void) { update_battery_display(); }
...
static void reset_volume_emphasis_timer_cb(lv_timer_t *timer) {
  ...
  if (s_volume_label_large) {
    lv_obj_set_style_text_color(s_volume_label_large, lv_color_hex(0xfafafa),
                                0); // Reset to white
  }
...
static void emphasize_volume_label(void) {
  ...
  if (s_volume_emphasis_timer) {
    ...
    s_volume_emphasis_timer =
        lv_timer_create(reset_volume_emphasis_timer_cb, 1500, NULL);
...
void ui_show_zone_picker(const char **zone_names, const char **zone_ids,
  ...
  s_zone_picker_count =
      (count > MAX_ZONE_PICKER_ZONES) ? MAX_ZONE_PICKER_ZONES : count;
  ...
  for (int i = 0; i < s_zone_picker_count; i++) {
    ...
    lv_obj_set_style_pad_ver(btn, 18,
                             0); // Larger vertical padding for easier tapping
    lv_obj_set_style_pad_hor(btn, 16, 0);
    lv_obj_set_height(btn, LV_SIZE_CONTENT);
    lv_obj_set_style_min_height(
        btn, 56, 0); // Minimum 56px tap target (Material Design guideline)
    ...
    // Icon font glyphs sit high in their bounding box - add top padding to
    // center visually
...
void ui_set_volume_with_range(float vol, float vol_min, float vol_max,
                              float vol_step) {
  os_mutex_lock(&s_state_lock);
  s_pending.volume = vol;
...
void ui_show_volume_change(float vol, float vol_step) {
  ...
  // Note: Reading volume_min/volume_max without lock (atomic float reads,
  // self-correct on next poll if stale)

  // Update volume arc immediately (optimistic)
  if (s_volume_arc) {
    int vol_pct = calculate_volume_percentage(vol, s_pending.volume_min,
                                              s_pending.volume_max);
    lv_arc_set_value(s_volume_arc, vol_pct);
  }

  // Update volume label immediately (optimistic)
  char vol_text[16];
  format_volume_text(vol_text, sizeof(vol_text), vol, s_pending.volume_min,
                     vol_step);
...
void ui_set_input_callback(ui_input_cb_t cb) { s_input_cb = cb; }
...
void ui_update(const char *line1, const char *line2, bool playing, float volume,
               float volume_min, float volume_max, float volume_step,
...
void ui_set_status(bool online) { ui_set_online(online); }
...
void ui_set_artwork(const char *image_key) {
  ...
  if (img_len != expected_rgb565_size) {
    ESP_LOGW(UI_TAG,
             "Unexpected image size: %zu bytes (expected %zu for %dx%d RGB565)",
  ...
  bool ok = ui_rgb565_from_buffer((const uint8_t *)img_data, SCREEN_SIZE,
                                  SCREEN_SIZE, &new_img);
  ...
  lv_obj_clear_flag(s_artwork_image, LV_OBJ_FLAG_HIDDEN);
  lv_obj_set_size(s_artwork_image, s_artwork_img.dsc.header.w,
...
int ui_zone_picker_get_selected(void) { return ui_get_zone_picker_selected(); }
...
void ui_zone_picker_scroll(int delta) {
  ...
  if (new_pos < 0)
    new_pos = s_zone_picker_count - 1;
  if (new_pos >= s_zone_picker_count)
    new_pos = 0;
...
void ui_set_update_available(const char *version) {
  if (version && version[0]) {
    ...
    if (!s_update_btn && s_ui_container) {
      ...
      lv_obj_set_style_bg_color(s_update_btn, lv_color_hex(0x4CAF50),
                                0); // Green
      ...
      lv_obj_add_event_cb(s_update_btn, update_btn_clicked, LV_EVENT_CLICKED,
                          NULL);
    }
    ...
    if (s_update_btn) {
      ...
      if (label) {
        char text[64];
        snprintf(text, sizeof(text), UI_ICON_DOWNLOAD " Update to %s",
                 s_update_version);
  ...
  if (s_update_btn) {
    ...
    if (label) {
      if (percent >= 0 && percent <= 100) {
        ...
        lv_label_set_text(label, text);
        lv_obj_set_style_bg_color(s_update_btn, lv_color_hex(0x2196F3),
                                  0); // Blue during update
        lv_obj_clear_flag(
            s_update_btn,
            LV_OBJ_FLAG_CLICKABLE); // Disable clicking during update
      } else if (percent < 0) {
        // Hide or reset
        lv_obj_add_flag(s_update_btn, LV_OBJ_FLAG_HIDDEN);
...
void ui_set_controls_visible(bool visible) {
  if (visible) {
    ...
    if (s_artwork_image)
      lv_obj_set_style_img_opa(s_artwork_image, LV_OPA_40, 0);
    ESP_LOGI(UI_TAG, "Controls shown");
    // Force battery display update after showing controls (GH-86)
    // Without this, hysteresis in update_battery_display() prevents the icon
    // from reappearing
