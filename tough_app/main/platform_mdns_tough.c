// platform_mdns.h implementation -- identical to frame_app/main/platform_mdns_frame.c
// Only change: device-info product name is "hiphi-tough". This backend is
...
bool platform_mdns_discover_base_url(char *out, size_t len) {
    ...
    for (mdns_result_t *r = results; r; r = r->next) {
        ...
        if (!found && r->addr && r->port &&
            ...
            ESP_LOGI(TAG, "Using IP:port: %s", url);
            found = true;
        }
    }
