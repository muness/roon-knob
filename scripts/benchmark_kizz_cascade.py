...
class StreamingModel:
    ...
    def invoke(self, chunk: np.ndarray) -> np.ndarray:
        self.interpreter.set_tensor(self.input["index"], chunk[None, ...])
        self.interpreter.invoke()
        return self.interpreter.get_tensor(self.output["index"])

    def scores(self, features: np.ndarray, stop_at: float | None = None) -> tuple[np.ndarray, float, int]:
        ...
        for index in range(chunks):
            ...
            output = self.invoke(chunk)
            score = float(output.reshape(-1)[0]) / 255.0
            values.append(score)
            if stop_at is not None and score >= stop_at:
                break
        elapsed_ms = (time.perf_counter_ns() - started) / 1_000_000.0
        return np.asarray(values, dtype=np.float32), elapsed_ms, len(values)
...
class OrderedDetector(StreamingModel):
    ...
    def scores(self, features: np.ndarray, stop_at: float | None = None) -> tuple[np.ndarray, float, int, np.ndarray]:
        quantized = quantize_features(features)
        chunks = len(quantized) // MODEL_STRIDE
        ...
        for index in range(chunks):
            chunk = quantized[index * MODEL_STRIDE : (index + 1) * MODEL_STRIDE]
            output = self.invoke(chunk).reshape(-1).astype(np.float64)
        ...
        return (np.asarray(probabilities, dtype=np.float32), elapsed_ms,
                len(probabilities),
                np.asarray(completion_scores, dtype=np.float32))
...
def run_case(detector_path: Path, verifier_path: Path, path: Path, args) -> dict:
    ...
    if args.detector_kind == "ordered":
        ...
        detector_scores, detector_ms, detector_invocations, completion_scores = detector.scores(
        ...
        detector_scores, detector_ms, detector_invocations = detector.scores(
    ...
    if detector_index is not None:
        ...
        verifier_features = micro_features(clip)
        ...
        verifier_scores, verifier_ms, verifier_invocations = verifier.scores(
