...
static void start_wifi_msg_alternation(const char *error, int attempt,
  ...
  snprintf(s_wifi_retry_msg, sizeof(s_wifi_retry_msg), "WiFi: Retry %d/%d",
           attempt, max);
...
void rk_net_evt_cb(rk_net_evt_t evt, const char *ip_opt) {
  ...
  case RK_NET_EVT_CONNECTING: {
    ...
    // Only show "Connecting..." on first attempt; during retries, keep showing
    // error/retry
    if (retry == 0) {
...
static void ui_loop_task(void *arg) {
  ...
  while (true) {
    ...
    // Check stack usage periodically (every 60 seconds = 6000 iterations at
    // 10ms)
    ...
    if (++stack_check_counter >= 6000) {
      ...
      ESP_LOGI(TAG, "ui_loop stack usage: %u/%u bytes (peak usage, %u free)",
               (unsigned int)used_bytes, UI_LOOP_STACK_SIZE,
...
void app_main(void) {
  ...
  // Create UI loop task BEFORE starting WiFi (WiFi events need LVGL task
  // running)
  ESP_LOGI(TAG, "Creating UI loop task");
  xTaskCreate(ui_loop_task, "ui_loop", UI_LOOP_STACK_SIZE, NULL, 2,
              &g_ui_task_handle); // 32KB stack (LVGL + gzip decompression)
  ...
  // Start WiFi AFTER UI task is running (WiFi event callbacks use
  // lv_async_call) Always start WiFi - we always boot into Roon mode now
