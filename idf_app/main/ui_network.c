...
static void show_reset_confirm_dialog(void) {
  ...
  lv_obj_set_style_bg_color(btn_reset, lv_color_hex(0xe53935),
                            LV_STATE_PRESSED);
  ...
  lv_obj_set_style_bg_color(btn_cancel, lv_color_hex(0x5a5a5a),
                            LV_STATE_PRESSED);
...
static void refresh_labels(void) {
  ...
  if (s_widgets.name_value) {
    if (cfg.knob_name[0]) {
      ...
      lv_label_set_text_fmt(s_widgets.name_value, "%s",
                            wifi_mgr_get_hostname());
    }
  }
...
static void set_ip_text(const char *text) {
  ...
  lv_label_set_text_fmt(s_widgets.ip_value, "%s",
                        (text && text[0]) ? text : "(no IP)");
}
...
static lv_obj_t *create_button(lv_obj_t *parent, const char *label,
                               lv_event_cb_t cb) {
...
static void ensure_panel(void) {
  ...
  lv_obj_set_width(s_widgets.bridge_value,
                   120); // Constrain width to enable scroll
...
bool ui_is_settings_visible(void) {
  return s_widgets.panel &&
         !lv_obj_has_flag(s_widgets.panel, LV_OBJ_FLAG_HIDDEN);
}
