...
namespace esphome
{
  namespace micro_wake_word
  {
    ...
    void MicroWakeWord::dump_config()
    {
      ...
      if (this->ordered_state_model_ != nullptr)
        this->ordered_state_model_->log_model_config();
...
void MicroWakeWord::add_ordered_state_model(
    const uint8_t *model_start, float probability_cutoff,
    size_t sliding_window_average_size, const std::string &wake_word,
    size_t tensor_arena_size) {
  this->ordered_state_model_ = make_unique<OrderedStateModel>(
      model_start, probability_cutoff, sliding_window_average_size, wake_word,
      tensor_arena_size);
}

void MicroWakeWord::set_wake_word_model_parameters(
    float probability_cutoff, size_t sliding_window_size) {
  if (this->ordered_state_model_ != nullptr) {
    this->ordered_state_model_->set_detection_parameters(probability_cutoff,
                                                         sliding_window_size);
    return;
  }
  for (auto &model : this->wake_word_models_)
    model.set_detection_parameters(probability_cutoff, sliding_window_size);
}

float MicroWakeWord::get_wake_word_probability() const {
  if (this->ordered_state_model_ != nullptr)
    return this->ordered_state_model_->ordered_probability();
  float probability = 0.0f;
  for (const auto &model : this->wake_word_models_)
    probability = std::max(probability, model.current_probability());
  return probability;
}

float MicroWakeWord::get_ordered_state_score() const {
  return this->ordered_state_model_ != nullptr
             ? this->ordered_state_model_->completion_score()
             : -INFINITY;
}
    ...
    bool MicroWakeWord::load_models_()
    {
      ...
      if (this->ordered_state_model_ != nullptr &&
          !this->ordered_state_model_->load_model(this->streaming_op_resolver_))
        return false;
#ifdef USE_MICRO_WAKE_WORD_VAD
    ...
    void MicroWakeWord::unload_models_()
    {
      ...
      if (this->ordered_state_model_ != nullptr)
        this->ordered_state_model_->unload_model();
#ifdef USE_MICRO_WAKE_WORD_VAD
    ...
    void MicroWakeWord::update_model_probabilities_()
    {
      ...
      if (this->ordered_state_model_ != nullptr)
        this->ordered_state_model_->perform_ordered_inference(audio_features);
#ifdef USE_MICRO_WAKE_WORD_VAD
    ...
    bool MicroWakeWord::detect_wake_words_()
    {
      ...
      if (this->ordered_state_model_ != nullptr &&
          this->ordered_state_model_->determine_detected())
      {
        this->detected_wake_word_ =
            this->ordered_state_model_->get_wake_word();
        return true;
      }
    ...
    void MicroWakeWord::reset_states_()
    {
      ...
      if (this->ordered_state_model_ != nullptr)
        this->ordered_state_model_->reset_decoder();
#ifdef USE_MICRO_WAKE_WORD_VAD
