...
class StreamingModel {
 public:
  ...
  /// @brief Returns the mean probability in the active detection window.
  float current_probability() const;
...
class WakeWordModel final : public StreamingModel {
 public:
  ...
  void set_detection_parameters(float probability_cutoff,
...
// A single acoustic model can emit ordered phone-state logits instead of a
// scalar wake probability. Phrase-level decoding remains in this runtime so
// deployment still owns one neural model and one tensor arena.
class OrderedStateModel final : public StreamingModel {
 public:
  OrderedStateModel(const uint8_t *model_start, float probability_cutoff,
                    size_t sliding_window_size, const std::string &wake_word,
  ...
  void set_detection_parameters(float probability_cutoff,
                                size_t sliding_window_size);
  void reset_decoder();
  float ordered_probability() const;
  float completion_score() const { return this->completion_score_; }
  const std::string &get_wake_word() const { return this->wake_word_; }
  bool perform_ordered_inference(
      const int8_t features[PREPROCESSOR_FEATURE_SIZE]);

 protected:
  std::string wake_word_;
  float completion_score_{-INFINITY};
  float ordered_score_cutoff_{0.0f};
  float baseline_probability_cutoff_{0.70f};
  float baseline_score_{16.167534f};
  float log_self_{-0.510825624f};
  float log_next_{-0.916290732f};
  float scores_[21];
};
